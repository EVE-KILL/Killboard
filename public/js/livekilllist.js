var generateLiveKillList = function (url, loadWebsocket, page, autoloadScroll) {
  var maxKillID = 0;
  ...
  var webSocket = function (websocketUrl, prependTo) {
    ...
    ws.onmessage = function (event) {
      ...
      // Only keep 100 elements in the killlist
      $(prependTo).children('tbody').children('tr').slice(100).remove();
      turnOnFunctions();
    };
  };
        ...
        if (isPreviousPageLoaded) {
          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              // Only keep 100 elements in the killlist
              $("#killlist").children('tbody').children('tr').slice(0,100).remove();
  ...
  $.ajax({
    ...
    url: currentOrigin + url + '/page/' + page,
    ...
    success: function (data) {
      ...
      if (loadWebsocket == true) {
        ...
        webSocket("wss://evedata.xyz/kills", "#killlist");
      }
