...
class CharacterAffiliation extends Cronjob
{
    ...
    private function processChunk(array $characters): void
    {
        ...
        if (!empty($updates)) {
            ...
            $characterIdsMinusUpdates = array_values(array_diff(
                array_map(fn ($char) => $char['character_id'], $characters),
                array_map(fn ($update) => $update['character_id'], $updates)
            )) ?? [];
